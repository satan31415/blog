...
# 通过 as 修改返回的列名
SELECT name as '名字' FROM student s 
# 带条件查询
SELECT name as '名字', age as '年龄' FROM student s WHERE age > 18
# 通过 and 连接多个搜索参数
SELECT name as '名字', age as '年龄' FROM student s WHERE age > 19 AND age <= 20
# 通过 like 做模糊查询
## '王%'：匹配以"王"开头的字符串。
## '%王'：匹配以"王"结尾的字符串。
## '%王%'：匹配包含"王"的字符串。
SELECT * FROM student s WHERE name LIKE '王%'
# 通过 in 来指定一个集合
SELECT * FROM student s WHERE class IN ('一班', '二班')
# 取反 not in
SELECT * FROM student s WHERE class NOT IN ('一班', '二班')
# 通过 between 来取一个区间
SELECT * FROM student s WHERE age BETWEEN 18 AND 20
# 通过 limit 分页查询(开始位置, 取的个数, 第二页就是 5,5)
SELECT * FROM student limit 0, 5
# 通过 order by 排序指定的列(DESC 降序, ASC 升序)
SELECT * FROM student s order by age DESC
SELECT * FROM student s order by age DESC,score asc
# 分组统计
